function lines = ModHoughLines(IMAGE,points2D,imageRes,theta,rho,peaks,fillgap,minlength)
...
%   $Revision: 1.1.8.11 $  $Date: 2011/12/22 18:25:46 $
...
numlines = 0; 
lines = struct([]);
% figure;plot(points2D(:,1),points2D(:,2),'.');axis equal; hold all;
for k = 1:size(peaks,1)
  ...
  for p = 1:length(idx) - 1
    ...
    if linelength_sq >= minlength_sq
      ...
      lines(numlines).IDX = LocatePointsInPC(points2D,[c r],imageRes);
%         if p1(1) > 1200 && p1(1) < 1220
%             metricOffset = min(points2D,[],1);
%             p1p2 = ([p1;p2]-1) * imageRes + repmat(metricOffset,size([p1;p2],1),1);
%             plot(points2D(lines(numlines).IDX,1),points2D(lines(numlines).IDX,2),'r.','markersize',10);hold on;
%                         plot(p1p2(:,1),p1p2(:,2),'go','markersize',10);
%                         keyboard;
%         end
    end
  end
...
%--------------------------------------------------------------------------
function idx = LocatePointsInPC(points2D,imagePos,imageRes)
% Find the indices of the points the the imagePos

% Locate the coordinates of the selected points
metricOffset = min(points2D,[],1);
metricPos = (imagePos-1) * imageRes + repmat(metricOffset,size(imagePos,1),1);
% Round to remove precision errors
metricPos = round(metricPos/0.001)*0.001;
points2D = round(points2D/0.001)*0.001;

% Find it in the point cloud
a = ismember(points2D(:,1),metricPos(:,1));
b = ismember(points2D(:,2),metricPos(:,2));
% [~,a] = ismember(points2D(:,1),metricPos(:,1));
% [~,b] = ismember(points2D(:,2),metricPos(:,2));
idx = find(a&b);
