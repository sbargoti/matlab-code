function [imageData, labelledImageData, labelData] = loadImagesLabelsMod(imageDataPath, labelDataPath, classColour)
...
skipFrameNo = 10; % not enough memoery to load everything
imageDataFull = zeros(imDim1,imDim2,nChannels,ceil(numIm/skipFrameNo));


tic;fprintf('Load Full Image set: \n');
for imNo = 1:ceil(numIm/skipFrameNo)    
%     currentImage = imread([imageDataPath fullFileList(imNo).name]); 
    currentImage = imread([imageDataPath fullFileList(imNo*skipFrameNo).name]); 
...
labelledImageData = zeros(imDim1,imDim2,3,numIm);
...
for imNo = 1:numIm
    ...
    currentImage = imread([imageDataPath fullFileList(labelIdx(imNo)).name]); 
    labelledImageData(:,:,:,imNo) = im2double(currentImage);
