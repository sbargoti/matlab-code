...
data = trainData;
theta = sae1Theta;
[sparseAutoencoderModel1] = sparseAutoencoderTrain(visibleSize, hiddenSize, ...
                                             lambda, sparsityParam, beta, data, theta);


% save sparseAutoencoderModel1 sparseAutoencoderModel1
% load sparseAutoencoderModel1                                         
sae1OptTheta = sparseAutoencoderModel1.opttheta;
...
[sae1Features] = feedForwardAutoencoder(sae1OptTheta, hiddenSizeL1, ...
                                        inputSize, trainData);
% min(min(sae1Features))= 3.3605e-06;
% max(max(sae1Features))= 0.9999;                                    
...
% max(max(sae2Features)) = 0.9869;
% min(min(sae2Features)) = 0.0043;
...
inputSizeSM = hiddenSizeL2;
numClasses = 10;
inputData = sae2Features;
labels = trainLabels;


[softmaxModel] = softmaxTrain(inputSizeSM, numClasses, lambda, inputData, labels);
...
stackedAEModel = stackedAETrain(inputSize, hiddenSizeL2, numClasses,...
    netconfig, lambda, trainData, trainLabels, stackedAETheta);
