...
beta = 0;              % weight of sparsity penalty term   (use denoise don't care sparse)    
...
noiseLevel = 0.3;
nElements = length(patches(:));
noiseIdx = datasample(1:nElements,nElements*noiseLevel,'Replace',false);
corruptPatches = patches;
corruptPatches(noiseIdx) = 0;
% [cost, grad] = sparseAutoencoderLinearCost(theta, debugvisibleSize, debugHiddenSize, ...
%                                            lambda, sparsityParam, beta, ...
%                                            patches);
[cost, grad] = denoiseAutoencoderLinearCost(theta, debugvisibleSize, debugHiddenSize, ...
                                           lambda, sparsityParam, beta, ...
                                           patches, corruptPatches);

% Check gradients
numGrad = computeNumericalGradient( @(x) denoiseAutoencoderLinearCost(x, debugvisibleSize, debugHiddenSize, ...
                                                  lambda, sparsityParam, beta, ...
                                                  patches, corruptPatches), theta);
...
figure;displayColorNetwork(corruptPatches(:, 1:100));
...
[sparseAutoencoderLinearModel] = denoiseAutoencoderLinearTrain(visibleSize, hiddenSize, ...
                                             lambda, sparsityParam, beta, patches, corruptPatches, noiseLevel, theta);
...
save('STL10DenoiseFeatures.mat', 'optTheta', 'ZCAWhite', 'meanPatch');
fprintf('Saved\n');
