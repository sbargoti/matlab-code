function convolvedFeatures = cnnConvolve(patchDim, numFeatures, images, W, b, ZCAWhite, meanPatch, CNNmethod)
...
% can do this normalisation in cpu
Wprep = W*ZCAWhite;
...
%     currentFeature = reshape(Wprep(featureNum,:),[patchDim patchDim 3]);
    currentFeature = reshape(Wprep(featureNum,:),[patchDim patchDim imageChannels]);
...
%       convolvedImage = fi(conv2(double(im),double(feature), CNNmethod),1,10,15)+convolvedImage;
...
targetIdx = (x<=-8);
sigm(targetIdx) = x(targetIdx)*0; 
targetIdx = (x>-8)&(x<=-1.6);
sigm(targetIdx) = (8-abs(x(targetIdx)))/64;
targetIdx = abs(x)<1.6;
sigm(targetIdx) = x(targetIdx)/4+0.5;
targetIdx = (x>=1.6)&(x<8);
sigm(targetIdx) = 1-(8-abs(x(targetIdx)))/64;
targetIdx = (x>8);
sigm(targetIdx) = ones(size(x(targetIdx)));
