...
% ufldlPath = 'H:\Work\code\deepLearning\UFLDL\code\';
...
% addpath( [ufldlPath 'dataSet\stl10_patches_100k']) % mnist digit data set
...
% Mod: show results
imageNameList = {'Airplane', 'Car', 'Cat', 'Dog'};
for i = 1: size(testImages,4)
    figure(1);imagesc(squeeze(testImages(:,:,:,i)))
    currentPredLabel = pred(i);
    currentTruthLabel = softmaxY(i);
    title(sprintf('Pred:%s, Truth:%s', imageNameList{currentPredLabel}, imageNameList{currentTruthLabel}));
