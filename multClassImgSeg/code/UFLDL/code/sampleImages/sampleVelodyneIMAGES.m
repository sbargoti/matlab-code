...
invalidPatches = logical(zeros(patchsize*patchsize*nband, numpatches));
...
%         if (entropy(imagePatch)>2) % grey band needs a lot lower entropy or it samples really slow
    ...
    invalidPatches(:,i) = invalidmaskPatch(:);
   
    if toc > 3
        fprintf('%i/%i ImageNumber:%i PixelLoc %i %i\n',i,numpatches,imageNo,coord1,coord2);
        tic;
    end
end
